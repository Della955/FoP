...
class StreamingService:
    ...
    def add_movie(self, movie_object):
        self._catalog.update({movie_object.get_title(): movie_object})
...
class StreamingGuide:
    ...
    def add_streaming_service(self, streaming_service_object):
        self._guide_list.append(streaming_service_object)

    def delete_streaming_service(self, streaming_service_name):
        # iterate over guide_list 
        # check each elemetns name, if the name matches
        # delete the object from the streaming services list 
        for elem in self._guide_list:
            if elem.get_name() == streaming_service_name:
                self._guide_list.remove(elem)
    
    def where_to_watch_movie (self, movie_title):
        watch_list = []
        #watch_list.append(movie_title)
        for streaming_obj in self._guide_list:
        ...
        #create a where to watch list 
        #iterate over streaming service guide (guide_list)
        #check movie title in each object of guide list 
        # if movie title exists in streaming service 
        # if list is empty return "none"
        # return the list 
    def get_streaming_service(self):
        return self._guide_list
